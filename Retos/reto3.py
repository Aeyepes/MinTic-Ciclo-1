...
def clima(datos):
    ...
    df = np.array(datos, dtype='object')

    df = np.reshape(df, (-1,7))
    ...
    repositorio = [temp_min, temp_max, precip_min, precip_max, max_dias_sol]
    validador = [np.min(temp_min), np.max(temp_max), np.min(precip_min), np.max(precip_max ), np.max(max_dias_sol)]
    resultado = [fechas_temp_min,fechas_temp_max, fechas_precip_min, fechas_precip_max, fechas_max_dias_sol]

    contador = 0
    for f in repositorio:
        for i in range(0,len(df)) :
            if f[i] == validador[contador] :
                resultado[contador].append(fechas[i])
        # print(salida[contador])
        contador += 1

    print(fechas_temp_min, fechas_temp_max, fechas_precip_min, fechas_precip_max, fechas_max_dias_sol, sep="\n")
